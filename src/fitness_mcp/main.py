...
class FitnessDataLoader:
    def __init__(self, data_file: str = "data/fit_t.tab", exp_desc_file: str = "data/exp_organism_Agro.txt"):
    ...
    def _load_condition_details(self):
    ...
    def _cached_search_genes(self, query: str, limit: int, data_version: float) -> List[Dict[str, Any]]:
    ...
    def _cached_get_conditions(self, condition_filter: Optional[str], data_version: float) -> List[str]:
    ...
    def interpret_fitness_score(self, fitness_score: float) -> Dict[str, Any]:
        ...
        if fitness_score < -0.1:
            ...
            effect = 'neutral'
            interpretation = "Gene knockout has minimal effect on fitness in this condition."
    ...
    def get_condition_details(self, condition_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'growth_conditions': {
                'media': details['media'],
                'temperature': details['temperature'] + '°C' if details['temperature'] else '',
                'pH': details['pH'],
            ...
            } if details['condition_1'] else None
        }
...
def find_essential_genes(
    ...
    for gene_id, gene_info in fitness_loader.genes.items():
        ...
        for condition_data in fitness_data['fitness_data']:
            if (condition_data['fitness'] is not None and 
                condition_data['fitness'] >= min_fitness_threshold):
                essential_conditions.append({
                    'condition': condition_data['condition'],
                    'fitness_score': condition_data['fitness'],
                    'description': condition_data.get('description', ''),
                    'interpretation': fitness_loader.interpret_fitness_score(condition_data['fitness'])
                })
...
def find_growth_inhibitor_genes(
    ...
    for gene_id, gene_info in fitness_loader.genes.items():
        ...
        for condition_data in fitness_data['fitness_data']:
            if (condition_data['fitness'] is not None and 
                condition_data['fitness'] <= max_fitness_threshold):
...
def analyze_gene_fitness(
    ...
    for item in fitness_data['fitness_data']:
        ...
        if fitness_val < -0.5:  # Gene knockout improves fitness (gene normally inhibits growth)
            gene_inhibits_growth.append(item)
        elif fitness_val > 0.5:  # Gene knockout reduces fitness (gene is beneficial/essential)

...
# PairsDataLoader for iterative module building - minimal implementation using MetadataRegistry
class PairsDataLoader:
    """Minimal PairsDataLoader for iterative module building using MetadataRegistry backend."""
    
    def __init__(self):
        pass
        
    def load_data(self):
        """Load data via MetadataRegistry."""
        metadata_registry.load_data()
    
    def get_conditions_for_gene(self, gene_id: str) -> List[Dict[str, Any]]:
        """Get conditions where gene has significant fitness effects."""
        metadata_registry.load_data()
        effects = metadata_registry.get_gene_fitness_effects(gene_id)
        return [{"condition": effect.condition_id, "value": effect.fitness_value} for effect in effects]
    
    def get_genes_for_condition(self, condition_id: str) -> List[Dict[str, Any]]:
        """Get genes with significant fitness effects in condition."""
        metadata_registry.load_data()
        effects = metadata_registry.get_condition_fitness_effects(condition_id)
        return [{"gene": effect.gene_id, "value": effect.fitness_value} for effect in effects]
...
def discover_functional_modules(num_modules: int = 10, max_size: int = 10, seed: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Get all possible gene-condition pairs from MetadataRegistry
    metadata_registry.load_data()
    all_pairs = []
    for effect in metadata_registry.fitness_effects:
        all_pairs.append((effect.gene_id, effect.condition_id))
    
    # Try building modules from random starting points
    modules = []
    used_starts = set()
    
    for i in range(min(num_modules, len(all_pairs))):
        if not all_pairs:
            break
            
        # Choose random starting pair
        import random
        available_pairs = [p for p in all_pairs if p not in used_starts]
        ...
        module = build_iterative_module(start_gene, start_condition, max_size)

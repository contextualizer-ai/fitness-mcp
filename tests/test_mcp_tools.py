    ...
    def test_find_growth_inhibitor_genes(self):
        ...
        with patch.object(fitness_loader, 'load_data'), \
            ...
            mock_get_fitness.return_value = {
                ...
                "fitness_data": [
                    {"condition": "cond1", "fitness": -1.2, "description": "Condition 1"}
                ]
            }
...
class TestComplexAnalysisFunctions:
    ...
    def test_find_essential_genes_realistic(self):
        ...
        def mock_get_gene_fitness(gene_id, condition_filter=None):
            if gene_id in self.genes_data or gene_id in [g["sysName"] for g in self.genes_data.values()]:
                # Handle sysName lookup
                if gene_id not in self.genes_data:
                    gene_id = next(k for k, v in self.genes_data.items() if v["sysName"] == gene_id)
        ...
        def mock_interpret_score(score):
            if score is None:
                ...
                return {"effect": "gene_benefits_growth", "interpretation": "Essential gene"}
            elif score <= -0.5:
                return {"effect": "gene_inhibits_growth", "interpretation": "Growth inhibitor"}
            else:
        ...
        with patch.object(fitness_loader, 'load_data'), \
            ...
            result = find_essential_genes(condition_filter="stress", min_fitness_threshold=0.5, limit=10)
    ...
    def test_find_growth_inhibitor_genes_realistic(self):
        ...
        with patch.object(fitness_loader, 'load_data'), \
            ...
            result = find_growth_inhibitor_genes(condition_filter="stress", max_fitness_threshold=-0.5, limit=10)
    ...
    def test_analyze_gene_fitness_with_filters(self):
        ...
        mock_fitness_data = {
            ...
            "fitness_data": [
                ...
                {"condition": "cond2", "fitness": -1.0},  # Should be excluded (below min)
                {"condition": "cond3", "fitness": 0.5},   # Should be excluded (above max)
        ...
        with patch.object(fitness_loader, 'get_gene_fitness') as mock_get:
            ...
            all_conditions = (analysis["conditions_where_gene_benefits_growth"] + 
                            analysis["conditions_where_gene_inhibits_growth"] + 
                            analysis["neutral_conditions"])
            
            assert len(all_conditions) == 3  # cond1, cond3, and cond4 (0.5 is within range)

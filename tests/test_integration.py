...
class TestIntegrationWithRealData:
    ...
    def setup_data_paths(self):
        ...
        self.has_fitness_data = (
            os.path.exists(self.fitness_file) and 
            os.path.exists(self.exp_file)
        )
        self.has_module_data = (
            os.path.exists(self.modules_file) and 
            os.path.exists(self.meta_file)
        )

    @pytest.mark.skipif(not os.path.exists("data/fit_t.tab"), reason="Real fitness data not available")
    ...
    @pytest.mark.skipif(not os.path.exists("data/RbTnSeq_modules_t1e-7.csv"), reason="Real module data not available")
    ...
    def test_search_genes_integration(self):
        ...
        with patch.object(fitness_loader, 'genes', mock_genes), \
             patch.object(fitness_loader, 'loaded', True):
            
            # Search by gene function
            results = search_genes("polymerase", limit=5)
            assert len(results) >= 1
            assert any("polymerase" in result["description"].lower() for result in results)
    ...
    def test_find_essential_genes_integration(self):
        ...
        mock_condition_details = {
            "stress_cond": {"short_desc": "Stress condition", "exp_group": "stress"}
        }
        ...
        def mock_get_gene_fitness(gene_id, condition_filter=None):
            ...
            for i, condition in enumerate(mock_conditions):
                if condition_filter is None or condition_filter.lower() in condition.lower():
        ...
        def mock_interpret_score(score):
            if score is None:
                ...
                return {"effect": "gene_benefits_growth", "interpretation": "Essential for growth"}
            else:
                return {"effect": "neutral", "interpretation": "Not essential"}
        
        with patch.object(fitness_loader, 'load_data'), \
             patch.object(fitness_loader, 'genes', mock_genes), \
             patch.object(fitness_loader, 'get_gene_fitness', side_effect=mock_get_gene_fitness), \
             patch.object(fitness_loader, 'interpret_fitness_score', side_effect=mock_interpret_score):
            
            results = find_essential_genes(condition_filter="stress", min_fitness_threshold=0.5, limit=10)
    ...
    def test_analyze_gene_fitness_integration(self):
        ...
        mock_fitness_data = {
            ...
            "fitness_data": [
                {"condition": "beneficial_cond", "fitness": 0.8},   # Gene benefits growth
                {"condition": "inhibitory_cond", "fitness": -0.8}, # Gene inhibits growth
    ...
    def test_module_integration(self):
        ...
        with patch.object(module_loader, 'get_modules_for_gene') as mock_get_modules, \
    ...
    def test_data_consistency_integration(self):
        ...
        with patch.object(fitness_loader, 'get_gene_info') as mock_info, \
